...
当前 Phase 1 只支持浏览和只读预览，不支持编辑、保存或未保存修改提示。支持 `.cpp`、`.cc`、`.c`、`.h`、`.hpp`、`.cmake`、`CMakeLists.txt`、`.txt`、`.json` 和 `.md` 的 UTF-8 文本；二进制、不支持的类型及超过 1 MiB 的文件会显示提示。文件树最多检查 2,000 个条目、32 层目录，达到上限会提示。

修改工作区路径会清空旧文件树与预览。外部生成或修改文件后可点击 `Refresh / 刷新` 重新加载。骨架、契约及外部代码保持受保护状态；缺少有效生成清单时不会将文件标为可编辑。本阶段不会改动工程文件或现有候选、构建和导出流程。
