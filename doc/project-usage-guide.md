...
新节点会出现在画布中央附近。拖动节点本体只移动节点；拖动空白画布会平移视图，平移过程中光标显示为闭合手形，松开左键立即结束。鼠标滚轮以鼠标所在位置为中心缩放，范围为 25% 到 300%；平移过程中滚轮缩放仍然可用。按住 Shift 或 Ctrl 拖动空白画布仍然是框选多个节点，按住 Ctrl 或 Shift 点击节点或连线也仍然用于增加或减少选择。画布右键菜单中的 `Fit View / 适应视图` 和 `Reset View / 重置视图` 可以随时框住全部节点或恢复初始视图。
...
打开 `View / 视图` → `Theme / 主题`，在两项之间选择：

- `Light / 亮色`：默认主题，与之前的界面一致
- `Dark / 暗色`：适合夜间和长时间编辑

切换立即生效，不需要重启：菜单栏、工具栏、状态栏、停靠面板、表单控件、按钮、输入框、表格、菜单、对话框，以及蓝图画布的网格、节点、端口和连线会同时更新为当前主题。当前主题在菜单中显示为选中状态，选择会写入本机 `QSettings`，下次启动自动恢复。切换界面语言不会改变当前主题，`Theme` 菜单的名称与两项文本会随语言一起翻译。

主题只影响界面配色，不会修改蓝图内容、蓝图中保存的颜色或生成代码；切换主题也不会产生撤销记录。

### 5.9 浏览工作区源码

在构建面板填写工作区根目录后，从 `View > Workspace Editor / 视图 > 工作区编辑器` 打开文件浏览面板。它只展示该工作区下的 `generated-project/`，不展示工作区的候选目录和生成清单。展开子目录并双击文本文件可查看源码及相对路径；关闭候选审核窗口后仍可从此入口打开。

有有效生成清单时，普通工程文本文件可在面板中编辑并点击 `Save / 保存` 写回；骨架、契约、外部代码及保护信息不可确认的文件仍为只读。修改后文件路径显示 `*`，切换文件或工作区、刷新、关闭面板或主窗口时会询问保存、放弃或取消。保存前会检查文件路径和磁盘版本，发现外部修改时不会直接覆盖。支持 `.cpp`、`.cc`、`.c`、`.h`、`.hpp`、`.cmake`、`CMakeLists.txt`、`.txt`、`.json` 和 `.md` 的 UTF-8 文本；二进制、不支持的类型及超过 1 MiB 的文件会显示提示。文件树最多检查 2,000 个条目、32 层目录，达到上限会提示。

切换工作区会加载新工作区的文件树；取消切换会保留原工作区和未保存内容。外部生成或修改文件后可点击 `Refresh / 刷新` 重新加载。本阶段不在构建或导出前自动处理未保存修改，也不改动候选审核流程或生成清单。
