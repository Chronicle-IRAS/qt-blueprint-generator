...
- 2026-09-24：完成 Issue #32 Phase 2，Workspace Editor 支持普通文本文件编辑与安全保存、未保存状态提示和切换/关闭确认；受保护文件继续只读，并覆盖中英文及主题切换回归。
- 2026-09-24：完成 Issue #32 Phase 1，新增 generated-project 文件树、UTF-8 只读预览与可重新打开的工作区编辑器入口，包含路径边界、受保护文件分类、中英文及工作区切换回归覆盖；编辑和保存留待后续阶段。
- 2026-09-22：完成 Issue #28，画布空白区域按住左键拖动平移视图（平移中显示闭合手形，松开即结束），拖动节点或端口时不再触发平移，按住 Shift/Ctrl 拖动空白区域仍为框选；滚轮改为以鼠标位置为中心缩放并保留 25%–300% 限制，平移中滚轮缩放可用；复用现有 Fit View / Reset View，并补充画布交互 GUI 回归测试。
- 2026-09-22：完成 Issue #39，新增独立暗色 palette 并在 `View / 视图 → Theme / 主题` 运行时切换 Light/Dark；切换后 Qt Palette、stylesheet 与画布网格、节点、连线同时刷新（连线笔色改为绘制时读取主题，候选审核窗口在主题变化时重建 diff 高亮），选择通过 `QSettings` 的 `ui/theme` 记忆并在下次启动恢复，未知值回退亮色，语言切换不影响已选主题；补充主题切换、对比度与画布渲染回归测试。
...
- 真实 DeepSeek GUI 链路已验证成功：使用临时虚构“整数加一”蓝图与已保存 Provider 设置（`deepseek-flash`），实际触发主窗口 Generate、保存候选并显示文件列表。第一批 `generation_4b25df6df9574bfe8163a7a6b6c83791` 为 `active`，1 个 pending 文件，0 个接受文件，没有执行模型代码。检查候选后补充了提示词中的实际契约 include 路径；该回归已先确认失败，再修正并通过独立复审。
- 2026-09-20：Issue #18 第一组生成控制器已完成。支持请求上下文快照、契约提示词、180 秒超时、脱敏错误分类、取消与过期结果隔离，仅保存待审核候选。独立质量审查发现的选择引用重入问题已通过失败测试复现并修复，复审无 Critical/Important；控制器 CTest 复验通过（13.02 秒）。候选审核与主窗口联测仍在收尾，整个 Issue 尚未完成。
- 2026-09-16：恢复 Issue #18 开发，确认上轮子代理均已停止。生成控制器、候选审核与主窗口接入已重新分配；候选列表空壳与主窗口生成入口的失败测试已确认。实现与完整验收仍在进行，尚未交付。
- 2026-09-15：用户已合并 PR #25；远端主分支合并提交 `9959917`，Issue #17 已关闭。
- 下一目标 Issue #18 已启动，远端标签为 `in-progress`；新分支 `feature/issue-18-ai-generation-review`，计划见 `doc/plans/2026-09-15-issue-18-generate-review.md`。当前先实现 GUI 生成控制，再实现候选审核与主窗口接入。

- #17 已通过 PR #25 交付并由用户合并（`Closes #17`），远端 Issue 已关闭并移除 `in-review` 标签。Task 4 提交为 `f6d7fc6`；设置、连接测试和双语界面提交依次为 `310da60`、`1228038`、`493d0bc`。
