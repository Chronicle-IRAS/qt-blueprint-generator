...
- 2026-09-22：完成 Issue #29，画布空白处、节点和连线分别提供右键菜单（添加节点/全选/适应视图/重置视图、编辑/生成/删除、删除连线）；菜单复用现有 QAction 与业务逻辑，删除统一走 `BlueprintScene::deleteSelectedItems()`，节点右键使未选中对象成为当前对象且保留原有多选，并补充菜单与语言切换回归测试。
